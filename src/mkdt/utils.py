...
from mkdt.downstream_datasets import Aircraft, Cub2011, Dogs
from mkdt.networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, ResNet10
...
def get_dataset(dataset, data_path, batch_size=1, subset=None, subset_size=None, args=None, set_seed=True):
    ...
    if dataset == 'cifar10':
        ...
        dst_train = datasets.Flowers102(data_path, split = 'train', download=True, transform=transform)  
        dst_test = datasets.Flowers102(data_path, split = 'test', download=True, transform=transform)
        # class_names = dst_train.classes
        # class_map = {x: x for x in range(num_classes)}
    ...
    if args.zca:
        ...
        zca_dir = "zca"
        zca_file = os.path.join(zca_dir, f"zca_{args.dataset}.pt")

        if not os.path.exists(zca_dir):
            os.makedirs(zca_dir)
        
        if os.path.isfile(zca_file):
            print(f"Loading ZCA matrix from file {zca_file}")
            zca = torch.load(zca_file)
        
        else:
            print("Train ZCA")
            zca = K.enhance.ZCAWhitening(eps=0.1, compute_inv=True)
            zca.fit(images)
            torch.save(zca, zca_file)
    ...
    return channel, im_size, num_classes, dst_train, dst_test, zca
...
def epoch_ftd(mode, dataloader, net, optimizer, criterion, args, aug, scheduler):
    loss_avg, num_exp = 0, 0
    net = net.to(args.device)
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        loss_train = epoch('train', trainloader, net, optimizer, criterion, args, aug=True)
