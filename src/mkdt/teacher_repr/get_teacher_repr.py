...
def main(args):
    ...
    teacher_model = None
    if args.load_directly:
        teacher_model = resnet34(ResNet34_Weights, progress=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', type=str, help='architecture of model', default="resnet18", choices=["resnet18", "resnet34", "resnet50"])
    parser.add_argument('--model_path', type=str, help='path to local copy of model provided by mixbt repository')
    parser.add_argument('--load_directly', action="store_true", help='load model directly, not into mixbt repo code')
